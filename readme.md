...
37. **WhatsHap 對性染色體會報 `PloidyError: Inconsistent ploidy (2 and 1)`**：ensemble 經二級 `bcftools +fixploidy` 後，男性 chrX 為 PAR diploid + 非PAR haploid（混合倍體），whatshap 要求單一染色體倍體一致 → 解析 chrX 時崩潰（跑完所有體染色體後才爆，會浪費數小時）。解法：phasing 只做體染色體(chr1-22)，chrX/chrY/chrM 一律 passthrough 不 phase（見 modules/phasing.nf 的 WHATSHAP_PHASE case 分支）。臨床 compound 幾乎都在體染色體，影響極小。
