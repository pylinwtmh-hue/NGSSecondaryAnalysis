...
41. **DV 與 HC 的 `FORMAT/AD` header `Number` 定義不一致 → norm/merge 把 AD 弄壞**：`BCFTOOLS_ENSEMBLE`
    合併 DeepVariant + HaplotypeCaller 時，bcftools 警告 `Trying to combine "AD" tag definitions of
    different lengths` —— 兩 caller 對 AD 的 header `Number` 不一樣（至少一邊不是 `Number=R`）。只要有一邊
    非 R，bcftools 就無法把 AD 依 allele 正確處理，造成兩種當機：
